...
namespace GhettosFirearmSDKv2.Drone
{
    ...
    public class Drone40 : MonoBehaviour
    {
        ...
        public bool gottaAddConstrains = true;
        ...
        {
            this.transform.Translate(new Vector3(leftHandY * speed, rightHandY * ySpeed, -rightHandX * speed));
            this.transform.Rotate(new Vector3(0, 0, -leftHandX * speed));
        }

        void FixedUpdate()
        {
            if (!active && beforeSpin && this.transform.position.y >= startHeight + activationHeight)
            {
                Activate();
            }
            else if (active)
            {
                ...
                camPivot.localEulerAngles = Vector3.Lerp(horizontalStart, horizontalEnd, currentHorizontal);
            }
        }
        ...
        public void MoveCamera(cameraDirections direction)
        {
            ...
            {
                currentHorizontal += camHorizontalSpeed;
            }
